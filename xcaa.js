...
//Fake Reply
  const ftroli = {key: {fromMe: true, participant: `${participants.id.split('@')[0]}@s.whatsapp.net`, ...(m.chat ? { remoteJid: "120363025016778449@g.us" } : {}) }, message: {"orderMessage": {"orderId": "174238614569481", "thumbnail": global.thumb2, "itemCount": 1, "status": "INQUIRY", "surface": "CATALOG", "message": `Hai ${pushname}`, "token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}
...
`
  ...
  buttons: buttons,
  headerType: 4,
  contextInfo:{externalAdReply: {showAdAttribution: true, title: `Hai ${pushname}`, body: `${wib}`, previewType: "PHOTO", thumbnailUrl: ``, thumbnail: global.thumb2, sourceUrl: global.linkgc}}
  }
...
case 'attp': case 'ttp': {
  if (!text) return m.reply(`*Contoh : ${prefix + command} Teks*`)
  try {
  await xcaa.sendMedia(m.chat, `https://xteam.xyz/${command}?file&text=${text}`, global.packname, global.author, m, {asSticker: true})
  } catch (err) {
  m.reply(mess.error)
  }
  }
   ...
   templateMessage: {
    ...
    hydratedFooterText: global.author,
     ...
     callButton: {
       ...
       phoneNumber: global.ownerNumber2
     }
  }, {
     quickReplyButton: {
...
case 'couple': case 'ppcp': {
  ...
  break
case 'asupan': case 'asupanloli': case 'bocil': case 'santuy': case 'geayubi': case 'ukhty': case 'storyanime': {
  m.reply(mess.wait)
  try {
